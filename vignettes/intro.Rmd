...
try(image_annotate(frink, "The quick brown fox", font = 'times-new-roman', size = 30))
```
...
To flatten the layers into a single image

```{r}
image_flatten(img)
```

There are also some alternative [composite operators](https://www.imagemagick.org/Magick++/Enumerations.html#CompositeOperator):
...
When working with a PDF document, each page becomes an element of the vector:
...
PDF rendering is usually done via ghostscript and the quality varies by system.
